...
description: "Locks in the Claudette/Clauda W/L Calibration Inevitability role for the thread. TRIGGER PATTERN: claud*_*_calibration_inevitability — invoke for claudette_w_calibration_inevitability (coding, Windows; default), claudette_l_calibration_inevitability (coding, Linux), clauda_w_calibration_inevitability (non-coding, Windows), clauda_l_calibration_inevitability (non-coding, Linux). Also invoke for paraphrases naming 'Calibration Inevitability' with claudette/clauda + W/L. After invocation: persona set for commit-trailer attribution; canonical role-def v02_I (2026-05-11) loaded; operating constraints active. Use when: any thread making excellence-as-floor structurally inevitable across LLM hosts, /asae modes, and output points — Doc 00 STRICT-5+ rule with NULL-on-asking bypass, /asae mode-decomposition restructure, per-output calibration ladder, cross-LLM Layer 1/Layer 2 cut, Wave 0 subagent-only authoring. Sibling-not-successor to BOTH Excellence Inevitability v01 AND Floor Inevitability v01; structural prevention of Excellence-Inevitability-v01 failure (tiered-rigor menus under wallclock pressure). Distinct from Excellence Inevitability (research-to-hardwiring upstream), Floor Inevitability (adversarial deploy-gate review), Code Debugger (catch-after), Value Genius (defend-post-hoc). Skip for: client work outside Martinez Methods; threads where Krystal directs a different role."
---
