...
A **bundle** is the set of artifacts that, together, fully specify a single coding initiative under the Dare-to-Rise methodology: typically one PRD, one TRD, one AVD, one TQCD, optionally one UXD, plus any references they pull in. A **bundle index** (file ID prefix `BIDX`) is the manifest that names the bundle, lists its artifacts with versions and hashes, and exposes a flat heading-prefix table for cross-artifact navigation and traceability.
...
- The internal structure of PRD/TRD/AVD/TQCD/UXD artifacts (covered by their templates).
...
├── TQCD_cold-read_2026-04-25_v01_I.md
...
| TQCD     | TQCD_cold-read_2026-04-25_v01_I.md    | v01     | I      | 9b6a3c...       |
...
| TQCD-3.2      | 3     | Acceptance criteria mapping   | TQCD     | #tqcd-3-2-acceptance-...      |
```
...
- Verify that every requirement in the PRD has a matching design section in the TRD and a matching test in the TQCD.
...
A traceability matrix showing which PRD requirements are covered by which TRD/AVD/TQCD sections. Format is flexible; one common form:

```
| PRD Req   | TRD Design     | AVD Risk(s) | TQCD Test(s)        | Status   |
|-----------|----------------|-------------|---------------------|----------|
| PRD-3.1   | TRD-4.1        | AVD-7.2     | TQCD-3.1, TQCD-3.4  | covered  |
| PRD-3.2   | TRD-4.2        | —           | TQCD-3.2            | covered  |
...
```
- **2026-04-25 v01 I:** Initial bundle opened. PRD/TRD/AVD/TQCD/UXD all in I status.
```
...
4. Every heading in every artifact has exactly one row in BIDX-3, and every row in BIDX-3 corresponds to an existing heading.
5. Every ID in BIDX-3 conforms to the heading-prefix grammar.
6. No ID appears twice in BIDX-3.
7. The bundle's version and status in BIDX-1 match the filename of the index itself.
...
| Artifact | File                              | Version | Status | SHA-256 |
|----------|-----------------------------------|---------|--------|---------|
| PRD      | PRD_cold-read_2026-04-25_v01_I.md | v01     | I      | a3f1b2  |
| TRD      | TRD_cold-read_2026-04-25_v01_I.md | v01     | I      | 7c4e9d  |
...
- **v01 (2026-04-26):** Initial schema. Five required sections. SHA-256 short as freshness marker. GAP marking allowed in cross-reference matrix.
