...
related_artifacts:
  ...
  - mm-claude-canonical/references/anti-fabrication.md (standalone anti-fabrication discipline reference — gate-88; supersedes the prior inline-only note)
  - mm-fm-taxonomy/docs/asae-aspect-reference-2026-05-31-v02.json (machine-readable A1-A21 enforcement map — gate-25)
  - mm-claude-canonical/scripts/test/v10-fixture-corpus/run-fixtures.sh (empirical v10 enforcement demonstration — gate-89)
---
...
2. **Tier-33 rater-agentId-line format gotcha.** The lib `agent_invocation_history.sh` (Hook Tier 33) counts raters by lines matching the regex `^\*\*Rater agentId:\*\*` — a line that begins **exactly** with `**Rater agentId:**`. A "fancy" rater-section restructure (e.g., `**Rater agentId (Wave 1):** … | (Wave 2):** …`) does NOT match → the lib counts **0 raters** → "rater count 00 < expected 2" REFUSE at strict-5. **Always give each rater section a clean standalone `**Rater agentId:** <id>` line** (you may add wave-detail lines below it). Evidence: gate-88 first-commit refusal on exactly this; fixed by adding the canonical line. (Verify before committing: `grep -cE "^\*\*Rater agentId:\*\*" <gate>` must equal your rater count.)
...
- `anti-fabrication.md` (standalone anti-fabrication discipline reference — gate-88; F1 cardinal refusal + A8.1/A8.2 + rater-spawn Rule A/B + cross-arch rig + adjudication + honest-enforced-vs-specced + recursive-stake) — `mm-claude-canonical/references/anti-fabrication.md`; principles also documented inline in "Audit-log structural requirements" + "Independent rater spawn" sections above; for non-Claude raters that have no Claude agentId, see the cross-architectural reference above
- `asae-aspect-reference-2026-05-31-v02.json` (machine-readable A1-A21 enforcement map — gate-25) — `mm-fm-taxonomy/docs/`
- v10 fixture corpus (empirical A14-A20 enforcement demonstration — gate-89) — `mm-claude-canonical/scripts/test/v10-fixture-corpus/`
- Hook v08 / v09 / v10 — `mm-claude-canonical/hooks/`
